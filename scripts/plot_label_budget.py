...
# Legend order follows the factorial, not dict insertion order.
ARM_ORDER = ["none", "node", "pretrain", "graph", "node_graph"]
...
def main() -> int:
    ...
    ap.add_argument("--no-title", action="store_true",
                    help="omit the embedded title; for figures whose caption carries it")
    args = ap.parse_args()
    ...
    ordered = sorted(curve, key=lambda a: ARM_ORDER.index(a) if a in ARM_ORDER else 99)
    for arm in ordered:
        series = curve[arm]
    ...
    ax.set_xlabel("labelled fraction of the scaffold training split (%)")
    ax.set_ylabel(f"test {key.upper()} (scaffold split)")
    if not args.no_title:
        ax.set_title(f"{m['dataset']}: {key.upper()} vs labelled fraction\n"
